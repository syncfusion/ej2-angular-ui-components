...
- `#I393057` - Annotation Comment Filter – Introduced annotation filter support in the PDF Viewer, enabling users to efficiently manage and view annotations by applying filters based on specific criteria. The filter options can be accessed through the filter icon in the comments panel, allowing users to display only the annotations that match their requirements. Explore the documentation [here](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/angular/annotation/annotation-comment-filter)
...
- Starting with this release `(v34.1.29)`, no new features will be added to the Server-backed mode, as all the PDF Viewer functionalities are now available in the `Standalone` mode. If you are currently using the server-backed mode, please refer to the [migration documentation](https://help.syncfusion.com/document-processing/pdf/pdf-viewer/angular/server-to-standalone) and migrate to the `Standalone` PDF Viewer.
